...
export class OrderGateway {
  ...
  async payOder(@MessageBody() data, client: Socket) {
    this.server.emit('payOder', data);
  }

  @SubscribeMessage('exitPayment')
  async exitPayment(@MessageBody() data, client: Socket) {
    this.server.emit('exitPayment', data);
  }
